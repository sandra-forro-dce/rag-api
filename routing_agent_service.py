...
def fine_tuned(state: State):
    import requests
    fine_tuned_url=global_config["ft_url"]
...
def rag(state: State):
    ...
    response = requests.post(global_config["rag_url"], params=data)
    #print(response.text, '\n')
...
def main():
    ...
    parser.add_argument("--server_url", required=True, help="First URL: ip:port")
    parser.add_argument("--ft_url", required=True, help="Second URL: http://ip:port/endpoint")
    parser.add_argument("--rag_url", required=True, help="Third URL: http://ip:port/endpoint")
